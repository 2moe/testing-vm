...
static STATE: Lazy<app::GlobalState<State>> =
    Lazy::new(app::GlobalState::<State>::get);
...
fn menu_cb(m: &mut impl MenuExt) {
    if let Ok(mpath) = m.item_pathname(None) {
        ...
        match mpath.as_str() {
            ...
            "&File/Open...\t" => {
                if let Some(c) =
                    nfc_get_file(dialog::NativeFileChooserType::BrowseFile)
                {
                    if let Ok(text) = std::fs::read_to_string(&c) {
                        STATE.with(move |s| {
                            s.buf.set_text(&text);
                            s.saved = false;
                            s.current_file.clone_from(&c);
                        });
                    }
                }
            }
            ...
            "&File/Save as...\t" => {
                if let Some(c) =
                    nfc_get_file(dialog::NativeFileChooserType::BrowseSaveFile)
                {
                    STATE.with(move |s| {
            ...
            "&Edit/Copy\t" => ed.copy(),
            "&Edit/Paste\t" => ed.paste(),
            "&Help/About\t" => dialog::message_default(
                "A minimal text editor written using fltk-rs!",
            ),
            _ => unreachable!(),
        }
    }
